...
class AvitoSpider(scrapy.Spider):
    ...
    def ads_parse(self, response):
        result = {}
        result['title'] = response.xpath('//h1[@class="title-info-title"]/span[@class="title-info-title-text"]').extract()[-1].split('>')[1].split('<')[0]
        result['url'] = response.url
        result['photo'] = self.find_photo_src(response.xpath('//div[@class="item-view-content"]').extract()[-1])
        result['price'] = response.xpath('//div[@class="item-price"]//span[@itemprop="price"]').extract()[-1].split('"')[5]
        result['address'] = response.xpath('//div[@itemprop="address"]/span[@class="item-address__string"]').extract()[-1].split('\n')[1]
